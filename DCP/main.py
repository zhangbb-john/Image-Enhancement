...
def getAtomsphericLight(darkChannel, img, meanMode=False, percent=0.001):
    ...
    return atomsphericLight,nodes[i].x,nodes[i].y
...
def getRecoverScene(img, omega=0.95, t0=0.1, blockSize=15, meanMode=False, percent=0.001):
    ...
    atomsphericLight,Ax,Ay = getAtomsphericLight(imgDark, img, meanMode=meanMode, percent=percent)
...
path =  "E://desktop//recent_files//graduate_design//myfiles//03reading_paper//metrics//ssim//img_select//result"
files = os.listdir(path)
files =  natsort.natsorted(files)
num=50
for i in range(min([len(files),num])):
    file = files[i]
    filepath = path + "/" + file
    prefix = file.split('.')[0]
    if os.path.isfile(filepath):
        ...
        cv2.imwrite('OutputDepth/' + prefix + '_DCP_TM.jpg', np.uint8(transmission * 255))
